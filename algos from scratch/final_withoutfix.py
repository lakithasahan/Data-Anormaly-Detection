...
e=0
l=10
data=normalised_input_data[0:100]
data2=normalised_input_data[0:100]
target_=target[0:100]
data3=normalised_input_data[5:10]

data_=[]
for c in range(len(data2)):
	data_.append(np.append(data2[c], c))
	#normalised_input_data[c].append(c)
...
plt.subplot(2, 1, 1)
plt.scatter(data2[:,0], data2[:,1],c=output_array, cmap='Paired')
plt.title("IFOREST anormaly detected for the Mulcross data")

plt.subplot(2, 1, 2)
plt.scatter(data2[:,0], data2[:,1],c=target_, cmap='Paired')
